...
- [ ] Focus order matches visual order. DOM order drives Tab order, but `flex-direction: row-reverse`/`column-reverse`, `order`, and absolute positioning change only what is seen. Before placing a new control "before X", read the container's styles — in a `row-reverse` holder, first in the DOM is visually LAST.
...
- [ ] Contrast is measured against the **composited background the element actually renders on**, not the theme's default surface. See "Measuring contrast" below — a ratio computed against `bgwhite` is a guess until the real surface is resolved.
...
1. **Resolve the real surface for every place the element renders.** Walk up from the element to the first opaque background. Look for `!important` overrides, hover/selected/expanded states (`AccordionSummary`, table row hover, selected list items), and `rgba()` overlays — `rgba(255,255,255,.23)` over a `#1F1F1F` card is about `#535353`, not `#1F1F1F`. A component reused in two views (e.g. a flat list and an accordion) has two backgrounds; check both.
...
3. **Opacity multiplies and cannot be undone by a child.** An ancestor with `opacity: 0.5` halves every descendant's effective contrast, and `opacity: 1` on the child does nothing. The fix is to move the indicator out from under the translucent ancestor, or apply the dimming to the text only — never "reset" it on the child.
...
A regression test that asserts a token against the theme's default surface only proves the token — it will pass while the rendered element still fails. Assert against each resolved surface.
...
- **Per criterion, what the inventory must cover:**
  ...
  - **2.4.5 Multiple Ways** — the unit is the *set of pages*, not the sidebar. Diff the router's route table against every navigation mechanism (sidebar, breadcrumbs, search, sitemap, in-page links). Each page needs two ways in, except pages that are a step in a process or the result of one. For each route left with one way, cover it or state which exemption applies.
  - For any other criterion, read its definition and exceptions before scoping; don't infer scope from the ticket title.
- **Scope choices are questions, not decisions.** Which pages count, whether a v2 of the module is included, whether dark mode is supported, which tool satisfies "passes automated checks" — if the ticket doesn't say, record it as an open question with the default you assume, and list what you deliberately left out. Do not silently pick.
- **Acceptance criteria that cannot be met as written** (axe has no rule for 2.4.5; a screen reader cannot verify a contrast ratio) are called out as such, with what will be shown instead.
...
5. For contrast, confirm the colours axe (or your own check) used: read `getComputedStyle(el).color` / `backgroundColor` up the ancestor chain to the first opaque background, and multiply the `opacity` of every ancestor. Do this in each theme mode and in each view the component appears in (e.g. flat vs accordion).
