...
**Name the account each scenario runs as, and confirm that account can reach the screen.** A role-based scenario — "the accountant sees X", "a non-admin is denied Y", "the approver can edit Z" — is only executable if an account holding that role exists here *and* can open the page. Check before you write the case; do not assume the role exists because the ticket names it. If no such account exists, say plainly which fixture has to be created first. If you cannot run that check from where you are — no live environment, no database, or a prompt that forbids running the app — do not drop the case and do not assume the role exists. Write it with the account named in its precondition, and say in that same precondition that the account's existence is unverified from here.

A case nobody can execute does not come back as "not run" — it comes back as a **failure**, and it reads exactly like a real permissions bug. On ticket 244, TC-10 tested what an "advanced accountant" could see on a Deel Salary Person admin page. There are 38 non-superuser staff accounts on that database and not one of them can open that page at all, so the case never exercised the branch's code. It was recorded as a failing case and helped stop the ticket.
