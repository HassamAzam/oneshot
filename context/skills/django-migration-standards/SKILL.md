...
The `migration-standards` hook blocks a file that mixes a schema operation with `RunPython`, so you will not ship one by accident. What it leaves you: when a task needs both, decide the order — schema first, data second — and make the data migration depend on the schema one. A squash is exempt; `squashmigrations` concatenates whatever history held, and that is the tool working correctly.

## Data Migration Best Practices

- `Model._base_manager` inside `RunPython` is enforced by the hook — it blocks `.objects` and any `*_objects` manager. Worth knowing why, because the failure is silent: `apps.get_model()` does not expose custom managers unless they set `use_in_migrations = True` (this repo never does), and the default manager can still filter, so `active_objects` skips every soft-deleted row and the migration reports success having missed them.
...
The hook checks that it names the app's highest-numbered migration, so a stale
or missing pointer blocks the write rather than failing `manage.py check` with
`dlm.E004` for whoever pulls next. One line, the newest migration's filename
without `.py`; when a task adds several to one app it points at the
