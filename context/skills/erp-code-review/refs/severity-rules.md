...
Shared by all ERP code review agents (`backend-reviewer-agent`, `frontend-reviewer-agent`, `accessibility-reviewer-agent`, `util-reuse-agent`, `spec-conformance-agent`) and the `erp-code-review` skill orchestrator.

## Severity labels

| Label | Use for |
|---|---|
| **BLOCKER** | Security (auth, PII leak, injection, secrets, XSS), data loss, migration that breaks prod, missing GitLab ticket, broken build/tests, LSP violations that break existing callers, **missing must-have ticket requirement, behavior that contradicts the ticket, ticket-named edge case unhandled, ticket-named tests missing, scope creep into auth / payroll / migrations / shared utils**, **accessibility: keyboard trap, no accessible name on an interactive control, color-only status on an approval/payroll/leave workflow, unlabeled form field, live region rendered outside an open overlay (silent by construction)** |
