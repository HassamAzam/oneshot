...
description: Use when the user asks to review code, a diff, an MR, or uncommitted changes in this ERP repo. Orchestrates layer-specific review agents (backend-reviewer-agent, frontend-reviewer-agent, accessibility-reviewer-agent, util-reuse-agent) in parallel, then aggregates findings into a paste-ready GitLab review with severity labels and impact analysis. Grounded in `.claude/rules/` directory standards. Do NOT use for generic code review outside this repo.
---
...
| Any frontend file touched that renders JSX (components, containers — not pure `utils/`/selectors/reducers) | `accessibility-reviewer-agent` |
...
1. Concatenate findings in this order: spec conformance → backend → frontend → accessibility → util reuse → dead code
...
```
Diff is clean — no findings across backend, frontend, accessibility, util-reuse, dead-code, or spec-match checks.
```

If `spec-conformance-agent` was skipped (no `ticket_context`), word it as:

```
Diff is clean — no findings across backend, frontend, accessibility, util-reuse, or dead-code checks. Spec match not verified (no ticket context).
```
...
- `.claude/skills/frontend-accessibility/SKILL.md` — a11y checklist, fix patterns, severity mapping (accessibility agent loads this; also usable standalone)
