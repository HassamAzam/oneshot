...
- **Any create/update serializer that writes a field gated by a business precondition must enforce that precondition itself.** Add a `validate()` / `validate_<field>()` checking `self.instance` (or the resolved target) against the real business condition — e.g. "only a Deel-salary person can have `is_hourly_deel_contractor=True`" — before accepting the write. Never rely on the view's permission classes or the frontend to have already checked this: permission classes answer "can this caller hit this endpoint," never "is this specific target id/value valid for the business rule," and a valid token + a different `id` via Postman bypasses any FE-side guard entirely.
