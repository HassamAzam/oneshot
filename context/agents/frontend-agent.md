...
- **Check the data before applying a data-dependent fix.** When a task asks you to apply a review fix that relies on a property of the data — a list `key` being unique and stable, an `id` existing, a field always being present — confirm it where the data is produced (the view / serializer / payload builder) before writing it. Words like "presumably unique" in the task are a claim, not a fact. If the property does not hold, do not apply the fix: stop and report which field fails and why (e.g. "`url` repeats across reminders; `detail` changes on refresh"), and what would make a correct fix possible.
