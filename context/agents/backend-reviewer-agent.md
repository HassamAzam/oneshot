...
4. **Serializer hygiene** — validation mixed with side effects (emails, other-model writes), list endpoint returning detail payload, sensitive fields (passwords, tokens, internal IDs) leaking to clients. Also: any create/update serializer writing a field gated by a business precondition (e.g. "only a Deel-salary person can have `is_hourly_deel_contractor=True`") must enforce that precondition itself via `validate()` / `validate_<field>()` against `self.instance` — permission classes answer "can this caller hit this endpoint," never "is this specific target id/value valid for the business rule," and a valid token + a different `id` via Postman bypasses any FE-side guard entirely. **SUGGESTION**, escalated to **BLOCKER** when the endpoint is unauthenticated or the field touches payroll/leaves/auth/payments directly (see `.claude/rules/security.md` High-Scrutiny Surfaces). Fix: "add a `validate()` method checking `self.instance` against the actual business condition before accepting the write."
