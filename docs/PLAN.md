...
| `testcase-brainstorm` | 3 | **Divergent** case generation, not a checklist. Enumerate happy / negative / edge / boundary / side-effect / regression / cross-module scenarios from the plan + AC; force a "what would a hostile QA try" pass; score each case by blast radius so `verify` and `qa` know what is worth the time. Output is the single shared case list phases 6, 7 and 11 all run. |
