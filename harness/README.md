...
- Layout mirrors a `.claude/` directory, so `ONESHOT_SKILLS_ROOT=~/Documents/oneshot/harness`
  seeds worktrees from here instead of `~/Documents/erp/.claude`
  (see `src/lib/claudedir.ts`). Oneshot's own skills stay in `../skills/` and are merged in at seed time.
